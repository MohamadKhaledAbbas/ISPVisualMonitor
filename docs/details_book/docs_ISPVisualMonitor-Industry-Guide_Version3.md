...
## 24. “Continue” note
Next chapters will go deeper into:
- database schema outline (tables/indexes, Timescale hypertables)
- alert rules design (rule DSL vs config UI)
- RCA algorithms (evidence scoring)
- PPPoE-focused features and customer impact modeling
- Docker Compose reference architecture (self-hosted baseline)
- agent mTLS and certificate lifecycle
- operational runbooks (backup/restore, upgrades, migrations)
- UI design patterns for map/graph/timeline
...
- alert rules engine design (DSL examples)
- RCA algorithm v1 implementation details and evidence scoring
- PPPoE-centric modeling (customers, sessions, service impact)
- agent security: mTLS, cert rotation, and secret distribution
- Docker Compose reference architecture (self-hosted) with profiles
- UI information architecture for NOC workflows
- operations: backup/restore, upgrades, migrations, SLOs
